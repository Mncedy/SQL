...
Select new_vaccinations, total_vaccinations, people_vaccinated, people_fully_vaccinated, positive_rate
From covidStats..CovidVaccinations
Where total_vaccinations <> 0 AND new_vaccinations is not null
Order By total_vaccinations asc;

Select DISTINCT(cv.location), SUM(CONVERT(decimal, cv.total_vaccinations)) OVER (PARTITION By cv.total_vaccinations) as TotalVacc, 
...
Where cv.total_vaccinations <> 0 AND cd.total_cases is not null
...
Select cv.total_tests, cd.location
From covidStats..CovidVaccinations cv 
	JOIN covidStats..CovidDeaths cd
	ON cv.location = cd.location
...
/* Excess Mortality:**  If available, explore data on excess mortality (deaths above expected levels) to understand the true impact of the pandemic beyond confirmed cases. 
...
/* Correlations and Causality:**  Investigate potential correlations between various factors and Covid-19 metrics (e.g., population density, healthcare access, socioeconomic indicators). 
	Visualize these correlations using scatter plots or heatmaps. Remember, correlation doesn't imply causation, so further analysis might be required to establish causal relationships. */
...
-- Correlation Analysis between Cases and Stringency Index**: Explore the correlation between COVID-19 cases and government stringency measures using the Oxford Stringency Index.
